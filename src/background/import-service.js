...
function blobFromConvertedPayload(payload) {
  ...
  const deserializedGifBytes = deserializeSerializedByteObject(payload.gifBuffer);
  if (deserializedGifBytes && deserializedGifBytes.length > 0) {
    return new Blob([deserializedGifBytes], { type: mimeType });
  }
  return null;
}
